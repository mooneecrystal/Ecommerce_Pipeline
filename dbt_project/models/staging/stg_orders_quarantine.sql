...
WITH hashed_source AS (
    ...
    FROM {{ source('internal_app', 'orders') }}
    -- Only process the bad rows
    WHERE load_date = '{{ var("execution_date") }}' AND {{ get_order_quality_logic() }} IS NOT NULL
)
...
SELECT 
    * EXCLUDE _rn,
    ...
    {{ get_order_quality_logic() }} as quarantine_reason
